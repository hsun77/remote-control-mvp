...
- 支持无人值守：被控端打开应用后自动共享，控制端可用固定设备 ID 连接
...
2. 两台机器都启动 desktop。
3. 被控端输入信令地址，比如 `ws://192.168.1.10:4141`。默认会自动开始共享，也可以手动点击 `Share this computer`。
4. 控制端输入同一个信令地址，然后输入被控端的 6 位房间码或固定设备 ID，点击 `Connect`。

## 无人值守访问

左侧 `Unattended access` 默认开启。被控端只要打开 `Remote Control MVP`，就会自动共享主屏幕并把固定设备 ID 注册到信令服务器。

控制端连接时可以直接输入被控端显示的固定设备 ID，例如：

```text
RC-ABCD-2345
```

信令地址、TURN 配置、快捷键映射和无人值守开关会自动保存。杭州电脑只要之前配置过一次信令地址，之后人在北京也可以通过固定设备 ID 连接。
